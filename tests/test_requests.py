...
from requests3.http_cookies import (
...
class TestRequests:
    ...
    def test_entry_points(self):
        ...
        requests3.HTTPSession
        requests3.HTTPSession().get
        requests3.HTTPSession().head
    ...
    def test_invalid_url(self, exception, url):
        with pytest.raises(exception):
            requests3.HTTPSession().get(url)

    def test_basic_building(self):
        req = requests3.Request()
    ...
    def test_no_content_length(self, httpbin, method):
        req = requests3.Request(method, httpbin(method.lower())).prepare()
    ...
    def test_empty_content_length(self, httpbin, method):
        req = requests3.Request(method, httpbin(method.lower()), data='').prepare()
        assert req.headers['Content-Length'] == '0'
    ...
    def test_override_content_length(self, httpbin):
        ...
        r = requests3.Request('POST', httpbin('post'), headers=headers).prepare()
        assert 'Content-Length' in r.headers
        assert r.headers['Content-Length'] == 'not zero'

    def test_path_is_not_double_encoded(self):
        request = requests3.Request('GET', "http://0.0.0.0/get/test case").prepare()
    ...
    def test_params_are_added_before_fragment(self, url, expected):
        request = requests3.Request('GET', url, params={"a": "b"}).prepare()
        assert request.url == expected

    def test_params_original_order_is_preserved_by_default(self):
        param_ordered_dict = OrderedDict((('z', 1), ('a', 1), ('k', 1), ('d', 1)))
        session = requests3.HTTPSession()
        request = requests3.Request('GET', 'http://example.com/', params=param_ordered_dict)
        prep = session.prepare_request(request)
        assert prep.url == 'http://example.com/?z=1&a=1&k=1&d=1'

    def test_params_bytes_are_encoded(self):
        request = requests3.Request('GET', 'http://example.com',
                                   params=b'test=foo').prepare()
        assert request.url == 'http://example.com/?test=foo'

    def test_binary_put(self):
        request = requests3.Request('PUT', 'http://example.com',
    ...
    def test_HTTP_302_ALLOW_REDIRECT_GET(self, httpbin):
        r = requests3.HTTPSession().get(httpbin('redirect', '1'))
        assert r.status_code == 200
        assert r.history[0].status_code == 302
        assert r.history[0].is_redirect

    def test_HTTP_307_ALLOW_REDIRECT_POST(self, httpbin):
        r = requests3.HTTPSession().request('post', httpbin('redirect-to'), data='test', params={'url': 'post', 'status_code': 307})
        assert r.status_code == 200
    ...
    def test_HTTP_307_ALLOW_REDIRECT_POST_WITH_SEEKABLE(self, httpbin):
        byte_str = b'test'
        r = requests3.HTTPSession().request('post', httpbin('redirect-to'), data=io.BytesIO(byte_str), params={'url': 'post', 'status_code': 307})
        assert r.status_code == 200
        assert r.history[0].status_code == 307
        assert r.history[0].is_redirect
    ...
    def test_HTTP_302_TOO_MANY_REDIRECTS(self, httpbin):
        try:
            requests3.HTTPSession().get(httpbin('relative-redirect', '50'))
    ...
    def test_http_301_changes_post_to_get(self, httpbin):
        r = requests3.HTTPSession().request('post', httpbin('status', '301'))
        assert r.status_code == 200
    ...
    def test_http_301_doesnt_change_head_to_get(self, httpbin):
        r = requests3.HTTPSession().request('head', httpbin('status', '301'), allow_redirects=True)
        print(r.content)
        assert r.status_code == 200
    ...
    def test_http_302_changes_post_to_get(self, httpbin):
        r = requests3.HTTPSession().request('post', httpbin('status', '302'))
        assert r.status_code == 200
        assert r.request.method == 'GET'
        assert r.history[0].status_code == 302
        assert r.history[0].is_redirect
    ...
    def test_http_302_doesnt_change_head_to_get(self, httpbin):
        r = requests3.HTTPSession().request('head', httpbin('status', '302'), allow_redirects=True)
        assert r.status_code == 200
        assert r.request.method == 'HEAD'
        assert r.history[0].status_code == 302
        assert r.history[0].is_redirect
    ...
    def test_http_303_changes_post_to_get(self, httpbin):
        r = requests3.HTTPSession().request('post', httpbin('status', '303'))
        assert r.status_code == 200
        assert r.request.method == 'GET'
    ...
    def test_http_303_doesnt_change_head_to_get(self, httpbin):
        r = requests3.HTTPSession().request('head', httpbin('status', '303'), allow_redirects=True)
        assert r.status_code == 200
        assert r.request.method == 'HEAD'
        assert r.history[0].status_code == 303
        assert r.history[0].is_redirect
    ...
    def test_HTTP_200_OK_GET_WITH_PARAMS(self, httpbin):
        ...
        r = requests3.HTTPSession().get(httpbin('user-agent'), headers=heads)
    ...
    def test_HTTP_200_OK_GET_WITH_MIXED_PARAMS(self, httpbin):
        ...
        r = requests3.HTTPSession().get(httpbin('get') + '?test=true', params={'q': 'test'}, headers=heads)
        assert r.status_code == 200
    ...
    def test_requests_in_history_are_not_overridden(self, httpbin):
        resp = requests3.HTTPSession().get(httpbin('redirect/3'))
    ...
    def test_history_is_always_a_list(self, httpbin):
        ...
        resp = requests3.HTTPSession().get(httpbin('get'))
        assert isinstance(resp.history, list)
        resp = requests3.HTTPSession().get(httpbin('redirect/1'))
        assert isinstance(resp.history, list)
    ...
    def test_headers_on_session_with_None_are_not_sent(self, httpbin):
        ...
        req = requests3.Request('GET', httpbin('get'))
        prep = ses.prepare_request(req)
    ...
    def test_headers_preserve_order(self, httpbin):
        ...
        headers['Fifth'] = '5'
        headers['Second'] = '222'
        req = requests3.Request('GET', httpbin('get'), headers=headers)
        prep = ses.prepare_request(req)
    ...
    def test_BASICAUTH_TUPLE_HTTP_200_OK_GET(self, httpbin):
        ...
        r = requests3.HTTPSession().get(url, auth=auth)
        assert r.status_code == 200
    ...
    def test_set_basicauth(self, httpbin, username, password):
        ...
        r = requests3.Request('GET', url, auth=auth)
        p = r.prepare()
    ...
    def test_basicauth_encodes_byte_strings(self):
        ...
        r = requests3.Request('GET', 'http://localhost', auth=auth)
        p = r.prepare()
    ...
    def test_proxy_error_on_bad_url(self, httpbin, httpbin_secure):
        ...
        with pytest.raises(InvalidProxyURL):
            requests3.HTTPSession().get(httpbin(), proxies={'http': 'http://:8080'})

        with pytest.raises(InvalidProxyURL):
            requests3.HTTPSession().get(httpbin_secure(), proxies={'https': 'https://'})

        with pytest.raises(InvalidProxyURL):
            requests3.HTTPSession().get(httpbin(), proxies={'http': 'http:///example.com:8080'})
    ...
    def test_basicauth_with_netrc(self, httpbin):
        ...
        try:
            ...
            r = requests3.HTTPSession().get(url, auth=wrong_auth)
            assert r.status_code == 401
    ...
    def test_DIGEST_STREAM(self, httpbin):
        ...
        for authtype in self.digest_auth_algo:
            ...
            r = requests3.HTTPSession().get(url, auth=auth, stream=True)
            assert r.raw.read() != b''

            r = requests3.HTTPSession().get(url, auth=auth, stream=False)
            assert r.raw.read() == b''
    ...
    def test_conflicting_post_params(self, httpbin):
        url = httpbin('post')
        with open('Pipfile') as f:
            pytest.raises(ValueError, "requests3.Request('post', url, data='[{\"some\": \"data\"}]', files={'some': f})")
            pytest.raises(ValueError, "requests3.Request('post', url, data=u('[{\"some\": \"data\"}]'), files={'some': f})")

    def test_request_ok_set(self, httpbin):
        r = requests3.HTTPSession().get(httpbin('status', '404'))
        assert not r.ok

    def test_status_raising(self, httpbin):
        r = requests3.HTTPSession().get(httpbin('status', '404'))
        with pytest.raises(requests3.exceptions.HTTPError):
            r.raise_for_status()

        r = requests3.HTTPSession().get(httpbin('status', '500'))
        assert not r.ok

    def test_decompress_gzip(self, httpbin):
        r = requests3.HTTPSession().get(httpbin('gzip'))
    ...
    def test_pyopenssl_redirect(self, httpbin_secure, httpbin_ca_bundle):
        requests3.HTTPSession().get(httpbin_secure('status', '301'), verify=httpbin_ca_bundle)

    def test_invalid_ca_certificate_path(self, httpbin_secure):
        INVALID_PATH = '/garbage'
        with pytest.raises(IOError) as e:
            requests3.HTTPSession().get(httpbin_secure(), verify=INVALID_PATH)
        assert str(e.value) == 'Could not find a suitable TLS CA certificate bundle, invalid path: {}'.format(INVALID_PATH)

    def test_invalid_ssl_certificate_files(self, httpbin_secure):
        INVALID_PATH = '/garbage'
        with pytest.raises(IOError) as e:
            requests3.HTTPSession().get(httpbin_secure(), cert=INVALID_PATH)
    ...
    def test_http_with_certificate(self, httpbin):
        r = requests3.HTTPSession().get(httpbin(), cert='.')
        assert r.status_code == 200

    def test_https_warnings(self, httpbin_secure, httpbin_ca_bundle):
        """warnings are emitted with requests3.HTTPSession().get"""
        ...
        with pytest.warns(None) as warning_records:
            ...
            requests3.HTTPSession().get(httpbin_secure('status', '200'),
    ...
    def test_urlencoded_get_query_multivalued_param(self, httpbin):
        ...
        r = requests3.HTTPSession().get(httpbin('get'), params={'test': ['foo', 'baz']})
        assert r.status_code == 200
        assert r.url == httpbin('get?test=foo&test=baz')

    def test_form_encoded_post_query_multivalued_element(self, httpbin):
        r = requests3.Request(method='POST', url=httpbin('post'),
                             data=dict(test=['foo', 'baz']))
        prep = r.prepare()
        assert prep.body == 'test=foo&test=baz'

    def test_different_encodings_dont_break_post(self, httpbin):
        r = requests3.Request('post', httpbin('post'),
            data={'stuff': json.dumps({'a': 123})},
            params={'blah': 'asdf1234'},
            files={'file': ('test_requests3.py', open(__file__, 'rb'))})
        assert r.status_code == 200
    ...
    def test_unicode_multipart_post_fieldnames(self, httpbin):
        ...
        r = requests3.Request(
            ...
            files={'file': ('test_requests3.py', open(filename, 'rb'))})
        prep = r.prepare()
    ...
    def test_unicode_method_name_with_request_object(self, httpbin):
        files = {'file': open(__file__, 'rb')}
        s = requests3.HTTPSession()
        req = requests3.Request(u('POST'), httpbin('post'), files=files)
    ...
    def test_non_prepared_request_error(self):
        s = requests3.HTTPSession()
        req = requests3.Request(u('POST'), '/')

        with pytest.raises(ValueError) as e:
            s.send(req)
        assert str(e.value) == 'You can only send Preparedrequests3.'

    def test_custom_content_type(self, httpbin):
        r = requests3.Request('post',
            httpbin('post'),
            data={'stuff': json.dumps({'a': 123})},
            files={
                'file1': ('test_requests3.py', open(__file__, 'rb')),
    ...
    def test_hook_receives_request_arguments(self, httpbin):
        ...
        s = requests3.HTTPSession()
        r = requests3.Request('GET', httpbin(), hooks={'response': hook})
    ...
    def test_session_hooks_are_used_with_no_request_hooks(self, httpbin):
        ...
        r = requests3.Request('GET', httpbin())
        prep = s.prepare_request(r)
    ...
    def test_session_hooks_are_overridden_by_request_hooks(self, httpbin):
        ...
        r = requests3.Request('GET', httpbin(), hooks={'response': [hook1]})
        prep = s.prepare_request(r)
    ...
    def test_prepared_request_hook(self, httpbin):
        ...
        req = requests3.Request('GET', httpbin(), hooks={'response': hook})
        prep = req.prepare()
    ...
    def test_prepared_from_session(self, httpbin):
        class DummyAuth(requests3.auth.AuthBase):
            def __call__(self, r):
                r.headers['Dummy-Auth-Test'] = 'dummy-auth-test-ok'
                return r

        req = requests3.Request('GET', httpbin('headers'))
        assert not req.auth
    ...
    def test_prepare_request_with_bytestring_url(self):
        req = requests3.Request('GET', b'https://httpbin.org/')
        s = requests3.HTTPSession()
        prep = s.prepare_request(req)
    ...
    def test_cookie_parameters(self):
        ...
        jar = requests3.cookies.RequestsCookieJar()
    ...
    def test_cookie_duplicate_names_different_domains(self):
        ...
        with pytest.raises(requests3.cookies.CookieConflictError):
            jar.get(key)
    ...
    def test_time_elapsed_blank(self, httpbin):
        r = requests3.HTTPSession().get(httpbin('get'))
        td = r.elapsed
    ...
    def test_response_reason_unicode_fallback(self):
        ...
        r.status_code = 500
        r.encoding = None
        with pytest.raises(requests3.exceptions.HTTPError) as e:
            r.raise_for_status()
    ...
    def test_prepared_request_is_pickleable(self, httpbin):
        p = requests3.Request('GET', httpbin('get')).prepare()
    ...
    def test_prepared_request_with_file_is_pickleable(self, httpbin):
        files = {'file': open(__file__, 'rb')}
        r = requests3.Request('POST', httpbin('post'), files=files)
        p = r.prepare()
    ...
    def test_prepared_request_with_hook_is_pickleable(self, httpbin):
        r = requests3.Request('GET', httpbin('get'), hooks=default_hooks())
        p = r.prepare()
    ...
    def test_cannot_send_unprepared_requests(self, httpbin):
        r = requests3.Request(url=httpbin())
        with pytest.raises(ValueError):
            requests3.HTTPSession().send(r)

    def test_http_error(self):
        error = requests3.exceptions.HTTPError()
        assert not error.response
        response = requests3.Response()
        error = requests3.exceptions.HTTPError(response=response)
        assert error.response == response
        error = requests3.exceptions.HTTPError('message', response=response)
    ...
    def test_uppercase_scheme_redirect(self, httpbin):
        ...
        r = requests3.HTTPSession().get(httpbin('redirect-to'), params={'url': url})
        assert r.status_code == 200
    ...
    def test_long_authinfo_in_url(self):
        ...
        r = requests3.Request('GET', url).prepare()
        assert r.url == url

    def test_header_keys_are_native(self, httpbin):
        headers = {u('unicode'): 'blah', 'byte'.encode('ascii'): 'blah'}
        r = requests3.Request('GET', httpbin('get'), headers=headers)
        p = r.prepare()
    ...
    def test_header_validation(self, httpbin):
        ...
        headers_ok = {'foo': 'bar baz qux',
                      ...
                      'baz': '',
                      'qux': '1'}
        r = requests3.HTTPSession().get(httpbin('get'), headers=headers_ok)
    ...
    def test_header_value_not_str(self, httpbin):
        ...
        with pytest.raises(InvalidHeader) as excinfo:
            r = requests3.HTTPSession().get(httpbin('get'), headers=headers_int)
        assert 'foo' in str(excinfo.value)
        # Test for dict
        with pytest.raises(InvalidHeader) as excinfo:
            r = requests3.HTTPSession().get(httpbin('get'), headers=headers_dict)
        assert 'bar' in str(excinfo.value)
        # Test for list
        with pytest.raises(InvalidHeader) as excinfo:
            r = requests3.HTTPSession().get(httpbin('get'), headers=headers_list)
    ...
    def test_header_no_return_chars(self, httpbin):
        ...
        with pytest.raises(InvalidHeader):
            r = requests3.HTTPSession().get(httpbin('get'), headers=headers_ret)
        # Test for line feed
        with pytest.raises(InvalidHeader):
            r = requests3.HTTPSession().get(httpbin('get'), headers=headers_lf)
        # Test for carriage return
        with pytest.raises(InvalidHeader):
            r = requests3.HTTPSession().get(httpbin('get'), headers=headers_cr)
    ...
    def test_header_no_leading_space(self, httpbin):
        ...
        # Test for whitespace
        with pytest.raises(InvalidHeader):
            r = requests3.HTTPSession().get(httpbin('get'), headers=headers_space)
        # Test for tab
        with pytest.raises(InvalidHeader):
            r = requests3.HTTPSession().get(httpbin('get'), headers=headers_tab)

    @pytest.mark.parametrize('files', ('foo', b'foo', bytearray(b'foo')))
    def test_can_send_objects_with_files(self, httpbin, files):
        data = {'a': 'this is a string'}
        files = {'b': files}
        r = requests3.Request('POST', httpbin('post'), data=data, files=files)
        p = r.prepare()
    ...
    def test_can_send_file_object_with_non_string_filename(self, httpbin):
        f = io.BytesIO()
        f.name = 2
        r = requests3.Request('POST', httpbin('post'), files={'f': f})
        p = r.prepare()
    ...
    def test_autoset_header_values_are_native(self, httpbin):
        ...
        length = '16'
        req = requests3.Request('POST', httpbin('post'), data=data)
        p = req.prepare()
    ...
    def test_nonhttp_schemes_dont_check_URLs(self):
        ...
        for test_url in test_urls:
            req = requests3.Request('GET', test_url)
            preq = req.prepare()
            assert test_url == preq.url

    def test_auth_is_stripped_on_http_downgrade(self, httpbin, httpbin_secure, httpbin_ca_bundle):
        r = requests3.HTTPSession().get(
    ...
    def test_auth_is_retained_for_redirect_on_host(self, httpbin):
        r = requests3.HTTPSession().get(httpbin('redirect/1'), auth=('user', 'pass'))
    ...
    def test_prepare_body_position_non_stream(self):
        data = b'the data'
        prep = requests3.Request('GET', 'http://example.com', data=data).prepare()
    ...
    def test_rewind_body(self):
        ...
        # rewind it back
        requests3.utils.rewind_body(prep)
        assert prep.body.read() == b'the data'
    ...
    def test_requests_history_is_saved(self, httpbin):
        r = requests3.HTTPSession().get(httpbin('redirect/5'))
    ...
    def test_response_iter_lines(self, httpbin):
        r = requests3.HTTPSession().get(httpbin('stream/4'), stream=True)
        assert r.status_code == 200
    ...
    def test_response_context_manager(self, httpbin):
        with requests3.HTTPSession().get(httpbin('stream/4'), stream=True) as response:
            assert isinstance(response, requests3.Response)
    ...
    def test_response_json_when_content_is_None(self, httpbin):
        r = requests3.HTTPSession().get(httpbin('/status/204'))
    ...
    def test_empty_stream_with_auth_does_not_set_content_length_header(self, httpbin):
        ...
        r = requests3.Request('POST', url, auth=auth, data=file_obj)
    ...
    def test_chunked_upload_does_not_set_content_length_header(self, httpbin):
        ...
        url = httpbin('post')
        r = requests3.Request('POST', url, data=data)
        prepared_request = r.prepare()
        assert 'Transfer-Encoding' in prepared_request.headers
        assert 'Content-Length' not in prepared_request.headers
    ...
    def test_custom_redirect_mixin(self, httpbin):
        ...
        Ensures a subclassed ``requests3.HTTPSession`` can handle a certain type of
        ...
        class CustomRedirectSession(requests3.HTTPSession):
...
class TestTimeout:
    ...
    def test_stream_timeout(self, httpbin):
        try:
            requests3.HTTPSession().get(httpbin('delay/10'), timeout=2.0)
        except requests3.exceptions.Timeout as e:
    ...
    def test_invalid_timeout(self, httpbin, timeout, error_text):
        with pytest.raises(ValueError) as e:
            requests3.HTTPSession().get(httpbin('get'), timeout=timeout)
    ...
    def test_none_timeout(self, httpbin, timeout):
        ...
        """
        r = requests3.HTTPSession().get(httpbin('get'), timeout=timeout)
        assert r.status_code == 200
    ...
    def test_read_timeout(self, httpbin, timeout):
        try:
            requests3.HTTPSession().get(httpbin('delay/10'), timeout=timeout)
    ...
    def test_connect_timeout(self, timeout):
        try:
            requests3.HTTPSession().get(TARPIT, timeout=timeout)
    ...
    def test_encoded_methods(self, httpbin):
        ...
        r = requests3.Request(b'GET', httpbin('get'))
        assert r.ok
...
def test_requests_are_updated_each_time(httpbin):
    ...
    prep = requests3.Request('POST', httpbin('post')).prepare()
...
class TestPreparingURLs(object):
    ...
    def test_preparing_url(self, url, expected):
        ...
        r = requests3.Request('GET', url=url)
        p = r.prepare()
    ...
    def test_preparing_bad_url(self, url):
        r = requests3.Request('GET', url=url)
        with pytest.raises(requests3.exceptions.InvalidURL):
            r.prepare()
    ...
    def test_url_mutation(self, input, expected):
        ...
        """
        r = requests3.Request('GET', url=input)
        p = r.prepare()
    ...
    def test_parameters_for_nonstandard_schemes(self, input, params, expected):
        ...
        """
        r = requests3.Request('GET', url=input, params=params)
        p = r.prepare()
        assert p.url == expected
